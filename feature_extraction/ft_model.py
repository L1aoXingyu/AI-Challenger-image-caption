...
class feature_model(nn.Module):
    ...
    def __init__(self,
        ...
        self.word2vec = nn.Embedding(vocab_size, embed_dim)
        self.rnn = nn.LSTM(
            input_size=embed_dim,
            hidden_size=hidden_size,
            num_layers=num_layers,
            dropout=dropout)
        self.classifer = nn.Linear(hidden_size, n_class)
    ...
    def get_hidden(self, bs):
        h0 = Variable(
            torch.zeros(self.num_layers, bs, self.hidden_size)).cuda()
        c0 = Variable(
            torch.zeros(self.num_layers, bs, self.hidden_size)).cuda()
        return (h0, c0)

    def sample(self, ft, end):
        """
